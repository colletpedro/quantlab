...
**Ponto de equity de `u` (emenda T13):** o registro da equity de `u` acontece **após o débito do borrow fee** do mesmo close — a equity de `u` é o valor ao FIM de `u`, incluindo o fee do próprio dia. É o que faz o termo final da identidade de §6 (`equity_final`) incluir TODOS os fees: se o registro fosse antes do débito, um short aberto na ÚLTIMA barra deixaria o último ponto da curva sem o fee do último close e a conciliação abriria exatamente nesse valor (caso de borda exposto pelo E2E da T13). A ordem marcar → fee → margem → consultar é preservada — o fee continua usando o close da marcação, a margem o caixa pós-fee.

**Custo de saída sobre short (emenda T13):** o `EXIT` sobre posição short (a venda que zera a posição — semântica Q2: o laço vende o que existe) cobra o custo sobre **|notional|**, a MESMA regra da cobertura canônica por `EXIT_SHORT` (SHT-02.1) — nunca sobre o notional com sinal: `cost_for` com notional negativo subcobra o custo e pode zerá-lo com `min_cost = 0`, fazendo a saída de um short custar menos que a de um long de mesmo tamanho (caso exposto pelo E2E da T13 — a venda de cobertura debitava `exit_cost = 0`).
...
| 0.1 (emenda T13) | 2026-08-16 | E2E do DoD (T13) expôs dois casos de borda do caminho short, corrigidos spec-first sem reabrir o gate 2: (1) o registro da equity de `u` passa a ser **após** o débito do borrow fee do close (§4) — a equity de `u` é o valor ao fim de `u` e o termo final da identidade de §6 inclui TODOS os fees (antes, um short aberto na última barra abria a conciliação exatamente no valor do último fee); (2) o `EXIT` sobre short (Q2 — a venda que zera) cobra custo sobre **\|notional\|**, mesma regra da cobertura canônica por `EXIT_SHORT` (antes, `cost_for` com notional negativo subcobrava e zerava o custo de saída com `min_cost = 0`). |
