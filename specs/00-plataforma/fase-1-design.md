...
**Definição de "bruto" (v0.8 — bug real da Fase 2a).** As barras em `bars` são o OHLCV **como negociado na data** — **pré-split**, sem nenhum ajuste por evento corporativo. O ajuste (splits + dividendos) é aplicado exclusivamente em `get_series(adjusted=True)` (§3.7). Atenção à armadilha do yfinance: `auto_adjust=False` **não** entrega bruto — ele aplica splits ao OHLC (só deixa dividendos fora). O provedor deve fazer o **back-out dos splits** (coluna `Stock Splits`: `raw[t] = split_adj[t] × Π r_i` para splits com data estritamente posterior a `t`) antes de devolver o frame; o volume já vem como negociado (sem ajuste) e não é tocado. Violação desta definição duplica a contagem de splits: o split entra uma vez embutido no "raw" e outra no ajuste — diagnóstico e correção na v0.8 do §3.7.

**Guarda permanente de consistência bruto×split (v0.8).** Para todo split em `corporate_actions` com barra no dia ex e na barra imediatamente anterior, vale `close_bruto(anterior) ≈ ratio × close_bruto(ex)` (a razão observada fica mais perto de `ratio` do que de `1`; dividendo introduz ruído pequeno, tolerado pelo critério de proximidade). A verificação roda como script (`scripts/verify_raw_consistency.py`, alvo `make verify-raw`) sobre a base real — a recomendação de sanidade cruzada do ADR-0003 vira guarda executável, não checagem só de desenvolvimento. Um ticker cujo raw viole a relação é erro de ingestão, não dado legítimo.
...
**Bug real — dupla contagem de splits no "raw" (v0.8, descoberto no E2E da Fase 2a).** A sanidade cruzada recomendada pelo ADR-0003 pegou um bug de janela (ADR-0004), mas não pegou este: `YFinanceProvider.fetch_prices` usava `auto_adjust=False` e gravava o resultado como se fosse bruto, quando o yfinance já havia aplicado os splits ao OHLC. O ajuste do §3.7 aplicava os splits de novo — fator duplicado nos tickers com split dentro da janela ingerida (AAPL 4:1, NVDA 40×, GOOGL 20×, AMZN 20×, NEE 4:1). Sintoma honesto: o benchmark 1/N da Fase 2a "comprou" 414.573 ações de NVDA (832× o capital) porque o preço ajustado de 2015 saiu em US$ 0,012 (fator 40× aplicado duas vezes). Correção em três partes: (1) **§3.1** — definição de "bruto" (pré-split, como negociado) + guarda permanente de consistência bruto×split; (2) **provider** — back-out dos splits (`raw[t] = split_adj[t] × Π r_i`, splits com data estritamente posterior a `t`), com teste de unidade de forma fechada; (3) **dados** — migração determinística das barras afetadas (mesma fórmula sobre o que está em `bars`, sem refetch), verificada pela guarda. Lição reiterada da v0.5: a sanidade cruzada precisa ser **permanente e executável**, não um passo de desenvolvimento — o formato do erro (fator constante) era exatamente o que a v0.5 ensinou a reconhecer.
...
| Versão | Data | Mudança |
|---|---|---|
| 0.10 | 2026-08-14 | Bug real de dupla contagem de splits, descoberto no E2E da Fase 2a (T18): `YFinanceProvider.fetch_prices` usava `auto_adjust=False`, que no yfinance **ainda aplica splits** ao OHLC, e gravava o resultado em `bars` como se fosse bruto; o ajuste do §3.7 aplicava os splits de novo. Afetados: AAPL, NVDA, GOOGL, AMZN, NEE (splits dentro da janela ingerida); sintoma: benchmark 1/N "comprou" 414.573 ações de NVDA (832× o capital) a preço ajustado US$ 0,012. Correção: (1) §3.1 — definição de "bruto" (pré-split, como negociado) + guarda permanente de consistência bruto×split (`scripts/verify_raw_consistency.py`, `make verify-raw`); (2) provider — back-out dos splits antes de devolver o frame, com teste de unidade de forma fechada; (3) dados — migração determinística das barras afetadas a partir do próprio `bars` (mesma fórmula, sem refetch), verificada pela guarda. |
